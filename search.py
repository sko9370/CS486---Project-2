        ...
        if currentState not in explored:
            ...
            for succ in problem.getSuccessors(currentState):
                if succ[0] not in explored:
                    priority = path + [succ[1]]
                    pq.push((succ[0], path + [succ[1]]), problem.getCostOfActions(priority))
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    if problem.isGoalState(problem.getStartState()):
        ...
        explored = set()
        pq = util.PriorityQueue()
        pq.push( (problem.getStartState(), list() ), heuristic(problem.getStartState(), problem) )
    ...
    while not pq.isEmpty():
        ...
        if currentState not in explored:
            ...
            for successor in problem.getSuccessors(currentState):
                if successor[0] not in explored:
                    pq.push( (successor[0], path + [successor[1]]), problem.getCostOfActions(path + [successor[1]]) + heuristic(successor[0], problem) )
